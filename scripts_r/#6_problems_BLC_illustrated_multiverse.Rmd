...
#clrs <- c('red','blue','yellow')
y_lim <- c(0,5000)
x_lim <- c(-.85,0.01)
tit_max_panel <- 'PDW max'
tit_mean_panel <- 'PDW mean'
x_lab <- 'SCR < 0 based on BLC'
```
...
# i.e, 118 subjects x 42 trials x 100 max approaches (1/2 of all approaches)
...
if(save_fig == 'yes'){
  tiff(here('figures/correction_problems/hist_below_zero.tiff'), units="in", width=8, height=6, res=300)
...
sum(df_TTP_max$scr == 0, na.rm = T) / n_TTP_resp
sum(df_TTP_max$scr > 0, na.rm = T) / n_TTP_resp
sum(is.na(df_TTP_max$scr)) / n_TTP_resp
...
df_TTP_max_stimType <- df_TTP_max %>% 
  ...
  count() %>% 
  data.frame()
#df_TTP_mean_stimType$p_n <- df_TTP_mean_stimType$p /sum(df_TTP_max$scr == 0, na.rm = T)
...
tit_plot <- 'TTP SCR \n based on post CS-MAX'

x_lab_TTP <- 'SCR based on TTP for BLC SCR < 0'
...
# tit_plot <- 'TTP SCR \n based on post CS-MAX log rc'
# hist_max_TTP_lr <- ggplot(df_TTP_max, aes(x = scr_log_rc, fill = stimType)) +
#   geom_histogram(binwidth = bin_w, alpha = .5, position = 'identity') +
...
# tit_plot <- 'TTP SCR \n based on post CS-MEAN log rc'
# hist_mean_TTP_lr <- ggplot(df_TTP_mean, aes(x = scr_log_rc, fill = stimType)) +
#   geom_histogram(binwidth = bin_w, alpha = .5, position = 'identity') +
#   scale_fill_manual(values = c('blue','red','black')) +
#   scale_y_continuous(limits = y_lim) +
#   scale_x_continuous(limits = x_lim) +
...
if(save_fig == 'yes'){
  tiff(here('figures/correction_problems/hist_below_zero_and_TTP.tiff'), units="in", width=8, height=6, res=300)
...
Thus, what we can infer from these plots (based on these 5 participants) is that on trials where larger baseline values appear than the post CS values, that in many cases these are scored as zero responses in TTP, but there are also cases where this is not the case.
...
dat$postCS_start_lab <- revalue(dat$postCS_start, c('0' = 'CSon', 
        '500' = 'CSon + .5s',
        '1000' = 'CSon + 1s',
        '2000' = 'CSon + 2s',
        '3000' = 'CSon + 3s') )

dat$postCS_end_lab <- revalue(dat$postCS_end, c('4500' = 'CSoff - 1.5s', 
        '5000' = 'CSoff - 1s',
        '6000' = 'CSoff',
        '7000' = 'CSoff + 1s',
        '8000' = 'CSoff + 2s') )
...
clrs <- c('red','blue','yellow') # colors correspond to order in scale fill manual
# plot
plot_onset_max <- dat %>%
  filter(max1mean0 == 'max') %>%
...
if(save_fig == 'yes'){
  tiff(here('figures/correction_problems/neg_drift_or_onset_peak_value.tiff'), units="in", width=6, height=4, res=300)
plot(plot_onset_max)
dev.off()
}
```
...
```{r}
# RM anova
anovaModelRM = aov(scr ~ preCS_Time*postCS_startTime*postCS_endTime*max1mean0 + Error(id), dat)
summary(anovaModelRM)


# linear mixed model
lmeModel = lmer(scr ~ preCS_Time*postCS_startTime*postCS_endTime*max1mean0 + (1|id), dat)
anova(lmeModel)
```
...
Negative drift? Response onset compared to post CS time window

Multiple responses, Same peak? Compare onset of selected peak?
