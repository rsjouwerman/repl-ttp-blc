...
save_fig <- 'no'
x_lab <- expression(paste('Krippendorfs ',alpha))
y_lab <- 'fear acquisition training trial'
```

### Reliability by Krippendorff

Krippendorfs alpha from the irr package gives approximately the same values as the custom made Krippendorffs alpha from Zapf. They differ only on th 4th or 5th decimal place.
...
source(here('scripts_r/Zapf_Krip_alpha_function.R'))

# remove TTP approaches from dat: approach %in% 1:10
# remove approach 10 for visualisation: approach %in% 1:9
dat <- dat %>%
  filter(approach %in% 1:10) %>%
  droplevels()
...
# loop through 42 trials (115 subjects * 12 methods = 1380 observations per df)
for (ind in 1:42){
  df <- dat_split_per_trial[[ind]] # trial 1, csm
  
  # # # dataframe for kripp.alpha function from irr package
  # df_1 <- df %>% pivot_wider(names_from = c(id, cs, number_cs), values_from =
  #                              scr_log_r, names_prefix = 'response_id') %>%
  #   select(-approach) %>%
  #   as.matrix()
  # 
  # ka_rank <- kripp.alpha(df_1, method = 'ordinal')
  # 
...
```{r Plot Krippendorffs alpha}
# update structure of ka_df
ka_df$trial <- factor(ka_df$trial, levels = ka_df$trial)
#ka_df$cs <- as.factor(ka_df$cs)

#save(ka_df, file = here('data/ka_df_over_all_approaches.Rdata'))
...
# save fig
if (save_fig == 'yes'){
tiff(here('figures/krippAlpha/kA_allBCapproaches.tiff'), units="in", width=16, height=12, res=300)
plot(p_kA)
dev.off()
...
#### Pairwise (i.e, approach wise) comparisons of Krippendorfs alpha
...
# 42 acquistition trials
dat_split_per_trial <- dat %>%
  select(id, approach, cs, number_cs, scr_log_r) %>%
  group_split(cs,number_cs)
...
# select 2 approaches to compare
  ...
  # dataframe for kripp alpha function from Zapf et al 2016
  df_2 <- df_xy %>% pivot_wider(names_from = approach, values_from = scr_log_r, names_prefix = 'approach') %>%
    select(-c(id,cs,number_cs)) %>%
    as.matrix()
  
  # compute, note nboot is 10 statt 1000, sonst dauert das ewig! For all 45 approach comparisons, I set nboot = 1 which does not make sense for calc a confidence interval off course, bc only 1 value is estimated
  # for 45 (unique approach combinations) and 42 trial = 1890 estimations, this took approx 20 mins to run
  k_zapf_rank <- k_alpha(df_2, scaling = 'ordinal', nboot = 10)
  
  # 
  # print(paste(df$cs[1], df$number_cs[1], sep = '_'))
  
  ka_df[ind,'trial'  ] <-  paste(df$cs[1], df$number_cs[1], sep = '_')
  ka_df[ind, 'stimType'] <- as.character(df$cs[1])
  ka_df[ind, 'kaZ_rank'] <- k_zapf_rank$est.alpha
  ka_df[ind, 'kaZ_rank_lc'] <- k_zapf_rank$ci.boot.alpha[1]
  ka_df[ind, 'kaZ_rank_uc'] <- k_zapf_rank$ci.boot.alpha[2]
  ka_df[ind, 'app_x'] <- app_x
  ka_df[ind, 'app_y'] <- app_y
  
  #name_ka_df <- paste('ka_df',app_x,app_y,sep = '_')
  #assign(name_ka_df, ka_df)
  
  # put in one large dataframe
  ka_df_all <- rbind(ka_df_all,ka_df)
...
#save(ka_df_all, file = here('data/kA_pairwiseComp.Rdata'))
...
```{r Plot Krippendorffs alpha}
ka_backup <- ka_df_all
ka_df_all$stimType <- as.factor(ka_df_all$stimType)
ka_df_all$trial <- factor(ka_df_all$trial, levels = unique(ka_df_all$trial))


#### try differnt arrangement
ka_df_all$trial_nr <- factor(gsub('^.*_','',ka_df_all$trial), levels = 1:14)
ka_per_comp <- ka_df_all %>% 
  group_split(app_x, app_y) 

for (ind in 1:45){
 df_temp <- ka_per_comp[[ind]] %>%
   data.frame()
 # df_temp$stimType <- as.factor(df_temp$stimType)
#  df_temp$trial <- factor(df_temp$trial, levels = df_temp$trial)
...
# save plot
if (save_fig == 'yes'){
tiff(here('figures/krippAlpha/ka_approach_wise_comp_matrix.tiff'), units="in", width=24, height=24, res=450)
plot(plot_upp_tri)
dev.off()
}
```
...
```{r}
# try if i can combine both plots
...
# save plot
if (save_fig == 'yes'){
jpeg(here('figures/krippAlpha/ka_combined_v2.jpg'), units="in", width=12, height=12, res=300)
plot(plot_combined)
dev.off()
}
```
