...
```{r Read in TTP data}
load(here('data/TTP2_Claudi_SCR118_PostProcess.Rdata'))
...
#120 approaches --> now 200 approaches
...
Make flow chart of analytic choices.
...
Run t-test on all 200 BC approaches and 1 TTP approach. Display results (i.e., effect size) in specification curve.
...
# we have 200 + 1 unique approaches
res_ttest_d$approach_num <-  1:nrow(res_ttest_d)
res_ttest_d[order(res_ttest_d$effsize),'approach_eff_order'] <-  1:nrow(res_ttest_d)
```

Run Bayesian t-test on all 200 BC approaches and 1 TTP approach. Extract effect size for specification curve. Note that the effect size in fact represent something similar as when plotting the BFs. Because the Bayesian t-test tests whether the effect size is different from 0.
...
for(ind in 1:n_appr){
  ...
  dat_dummy <- dat_acq_means %>% filter(stimType %in% c('csp','csm')) %>%
  ...
  bf_post <- posterior(bf, iterations = 100000) # 1000000
...
labs_bw <- c('1','2','4','10','TTP') # note that I removed BW: 3s
...
p1 <- res_ttest_d %>%
  ...
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) +
  scale_color_manual(name = 'BLW', values = clrs[-3], labels = labs_bw) +
  theme_classic() +
  theme(text = element_text(size=size_text),
        legend.position = 'top') +
  xlab(x_lab) +
...
p2 <- res_ttest_d %>%
  ...
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) +
  scale_color_manual(name = 'PDW-onset', values = clrs, labels = labs_pdw_on) +
  theme_classic() +
  theme(text = element_text(size=size_text),
        legend.position = 'top') +
  xlab(x_lab) +
...
p3 <- res_ttest_d %>%
  ...
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) +
  scale_color_manual(name = 'PDW-offset', values = clrs, labels = labs_pdw_off) +
  theme_classic() +
  theme(text = element_text(size=size_text),
        legend.position = 'top') +
  xlab(x_lab) +
...
s2 <- ggplot(res_ttest_d, aes(x = approach_eff_order, y = postCS_start, color = postCS_start)) +
  geom_point(shape = '|', size = size_point) + 
   scale_color_manual(values = rep('black', length(levels(res_ttest_d$postCS_start)) )) +
    scale_y_discrete(name = 'PDW-onset', labels = labs_pdw_on) +
  theme(axis.text.x = element_blank(),
        axis.title.x = element_blank(), 
        axis.ticks.x = element_blank(),
        panel.background = element_blank(),
...
s3 <- ggplot(res_ttest_d, aes(x = approach_eff_order, y = postCS_end, color = postCS_end)) +
  geom_point(shape = '|', size = size_point) + 
   scale_color_manual(values = rep('black', length(levels(res_ttest_d$postCS_end)) )) +
    scale_y_discrete(name = 'PDW-offset', labels = labs_pdw_off) +
  theme(axis.text.x = element_blank(),
        axis.title.x = element_blank(), 
        axis.ticks.x = element_blank(),
        panel.background = element_blank(),
...
# color coded spec curves next to each other 
p_spec_curves <-  (p1 / s_all) +  plot_layout(heights = c(2,1)) | 
...
ps1 <- p1 / (s1 +  scale_color_manual(values = clrs[-3])) / s2 / s3 / s4 
...
if (save_fig == 'yes'){
 tiff(here('figures/spec_curve_effsizes.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(p_spec_curves)
dev.off()

tiff(here('figures/NHST/spec_curve.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps0 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_preCS.tiff'), units="in", width= fig_w, height=fig_h, res=fig_res)
plot(ps1 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_postCS_start.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps2 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_postCS_end.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps3 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_meanMax.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
...
if (save_fig == 'yes'){
   png(here('figures/spec_curve_effsizesBF.png'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(p_spec_curvesBF)
dev.off()

 tiff(here('figures/spec_curve_effsizesBF.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(p_spec_curvesBF)
dev.off()

tiff(here('figures/NHST/spec_curve_BF.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps0 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_preCS_BF.tiff'), units="in", width= fig_w, height=fig_h, res=fig_res)
plot(ps1 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_postCS_start_BF.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps2 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_postCS_end_BF.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps3 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()

tiff(here('figures/NHST/spec_curve_meanMax_BF.tiff'), units="in", width=fig_w, height=fig_h, res=fig_res)
plot(ps4 + plot_layout(heights = c(2,1,1,1,1)))
dev.off()
}
